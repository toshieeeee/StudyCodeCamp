...
function num_validation($num){
  ...
  if(isset($num) !== TRUE || mb_strlen($num) === 0){
    ...
    $error[$attr] = $attr.'は5桁以内で入力してください';
...
function insert_pro_info_table($link,$param1,$param2,$param3,$param4){

  try{

    $sql = 'INSERT INTO pro_info_table(pro_name,pro_price,pro_image,pro_create_date,pro_status) VALUES (?,?,?,?,?)';
...
function insert_pro_num_table($link,$param){

  try{

    $sql = 'INSERT INTO pro_num_table(pro_num,pro_create_date) VALUES (?,?)';
    $stmt = $link->prepare($sql); 
...
function update_pro_num_table($link,$pro_id,$pro_num){

  try{

    $pro_update_date = date('Y-m-d H:i:s');
    $pro_num_update = $pro_num;
...
function update_pro_status_table($link,$pro_id,$pro_status){

  try{

    if(preg_match('/close/', $pro_status)){

      $pro_status = 0;
              
    } else if(preg_match('/show/', $pro_status)) {
